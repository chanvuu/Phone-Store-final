...
public class Supplier extends JPanel implements MouseListener, KeyListener{
	...
	private JTextArea supplierNoteText;
	...
	public void initComponent() {
		...
		GridBagConstraints gbc_supplierNoteText = new GridBagConstraints();
		gbc_supplierNoteText.weightx = 0.1;
		gbc_supplierNoteText.weighty = 0.1;
		gbc_supplierNoteText.gridwidth = 3;
		gbc_supplierNoteText.insets = new Insets(0, 5, 5, 20);
		gbc_supplierNoteText.fill = GridBagConstraints.BOTH;
		gbc_supplierNoteText.gridx = 0;
		gbc_supplierNoteText.gridy = 12;
		inputPanel.add(noteScroll, gbc_supplierNoteText);

		supplierNoteText = new JTextArea();
		supplierNoteText.setForeground(textColor);
		supplierNoteText.setFont(new Font("Times New Roman", Font.PLAIN, 12));
		supplierNoteText.setBorder(new LineBorder(borderColor ,2 ,true));
		noteScroll.setViewportView(supplierNoteText);
		supplierNoteText.setColumns(10);
	...
	public void loadDataTable(ArrayList<SupplierDTO> list) {
		...
		for(SupplierDTO s : list) {
			supplierModel.addRow(new Object[] {
				s.getSupplierId(), s.getSupplierName(), s.getSupplierPhone(), s.getSupplierAddress(), s.getSupplierEmail(), s.getSupplierNote()
			});
		}
	}
	...
	public void mouseClicked(MouseEvent e) {
		...
		if(e.getSource() == btnAdd) {
			if(Validation.isEmpty(supplierNameText.getText())) {				
			...
			else if(!Validation.isValidMobileNo(supplierPhoneText.getText())) {
	            ...
	            String supplierNote = supplierNoteText.getText().trim();
            ...
            int index = getRowSelected();
            if (index != -1) {
                if (Validation.isValidMobileNo(supplierPhoneText.getText())) {
                    JOptionPane.showMessageDialog(this, "Đề nghị điền số điện thoại đúng!");
                } 
                else if(!Validation.isEmail(supplierEmailText.getText())) {
    				JOptionPane.showMessageDialog(this, "Vui lòng đúng định dạng email");
    			}
			...
			supplierPhoneText.setText(listSupplier.get(index).getSupplierPhone());
			supplierAdressText.setText(listSupplier.get(index).getSupplierAddress());
			supplierEmailText.setText(listSupplier.get(index).getSupplierEmail());
			supplierNoteText.setText(listSupplier.get(index).getSupplierNote());
